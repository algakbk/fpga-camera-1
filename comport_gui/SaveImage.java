...
public class SaveImage {
  ...
  static public byte[] bayer_NearestNeighbor(final byte bayer[],
      final int width, final int height, final BAYER_PATTERN pattern) {
    ...
    // int width = sx;
    // int height = sy;
    // int blue = ((tile == BAYER_PATTERN_BGGR) || (tile == BAYER_PATTERN_GBRG))
    // ? -1 : 1;
    ...
    // height -= 2;
    //
    // for (; height > 0; height--, bayer_index += bayerStep, rgb_index +=
    // rgbStep) {
    // //int t0, t1;
  ...
  static public BufferedImage BayerByteArrayToImage(final byte bayer_ba[],
      final int width, final int height, final BAYER_PATTERN pattern)
      throws Exception {
    if (width < 1 || height < 1 || bayer_ba.length < (width * height)) {
      throw new Exception("Bad argumements ba.length(" + bayer_ba.length
          + ")  must be greater than width(" + width + ") X height(" + height
          + ")  =" + (width * height));
    }
  ...
  static public BufferedImage ByteArrayToImage(final byte ba[],
      final int width, final int height) throws Exception {
    if (width < 1 || height < 1 || ba.length < (width * height * 3)) {
      throw new Exception("Bad argumements ba.length(" + ba.length
          + ")  must be greater than width(" + width + ") X height(" + height
          + ") X 3 =" + (width * height * 3));
    }
  ...
  static public BufferedImage BayerByteArrayToImage(final byte ba[],
      final int width, final int height) throws Exception {
    if (width < 1 || height < 1 || ba.length < (width * height * 3)) {
      throw new Exception("Bad argumements ba.length(" + ba.length
          + ")  must be greater than width(" + width + ") X height(" + height
  ...
  static public void SaveImageAs(Image img, Component parent) {
    try {
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        System.out
            .println("You chose to save the image in this file: " + fpath);
        last_dir = chooser.getCurrentDirectory();
        SaveImage.SaveImageFileBySuffix(img, chooser.getSelectedFile()
            .getAbsolutePath());
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static void SaveImageFileBySuffix(Image img_in, String filename)
      throws IOException {
    final String suffixes[] = ImageIO.getWriterFileSuffixes();
    ...
    for (int i = 0; i < formats.length && !format_found; i++) {
      ...
      Iterator<ImageWriter> iw_by_suffix_it = ImageIO
      ...
      while (iw_by_suffix_it.hasNext() && !format_found) {
        ...
        // Iterator iw_by_format_it = ImageIO
        // .getImageWritersByFormatName(formats[i]);
        Iterator<ImageWriter> iw_by_format_it = ImageIO
            .getImageWritersByFormatName(formats[i]);
    ...
    BufferedImage img_to_record = new BufferedImage(new_width, new_height,
        BufferedImage.TYPE_INT_RGB);
    img_to_record.getGraphics().drawImage(img_in, (w_inc / 2), (h_inc / 2),
        null);
